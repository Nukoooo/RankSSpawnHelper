...
namespace RankSSpawnHelper.Ui.Widgets;
...
public static class Widget
{
    ...
    {
        return BeginFramedGroupInternal(label, Vector2.Zero, description);
    }

    public static float BeginFramedGroup(string label, Vector2 minSize, string description = "")
    {
        return BeginFramedGroupInternal(label, minSize, description);
    }
    ...
    private static float BeginFramedGroupInternal(string label, Vector2 minSize, string description)
    {
        ...
        using var style = ImRaii.ImRaii.PushStyle(ImGuiStyleVar.FramePadding, Vector2.Zero)
        ...
        // Label block
        ImGui.SameLine();
        using var group = ImRaii.ImRaii.Group();
